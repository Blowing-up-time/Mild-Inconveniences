...
import com.inconvenience.apkdecompiler.R;
import com.inconvenience.apkdecompiler.StaticShit;
...
public class ManifestAttributes {
	...
	private static final int ATTR_XML = R.raw.android_attrs;
	private static final int MANIFEST_ATTR_XML = R.raw.android_attrs_manifest;
	...
	private void parseAll() {
		parse(loadXML(ATTR_XML, "ATTR_XML"));
		parse(loadXML(MANIFEST_ATTR_XML, "MANIFEST_ATTR_XML"));
		LOG.debug("Loaded android attributes count: {}", attrMap.size());
	}

	private Document loadXML(int id, String xml) {
		LOG.debug("DOING UR MUM (by \"ur mum\" i mean the xml shit: " + xml + ")");
		Document doc;
		try (InputStream xmlStream = StaticShit.res.openRawResource(id)) {
			if (xmlStream == null) {
				throw new JadxRuntimeException(id + " not found in classpath");
			}
			DocumentBuilder dBuilder = null;
			dBuilder = XmlSecurity.getSecureDbf().newDocumentBuilder();
			doc = dBuilder.parse(xmlStream);
		} catch (ParserConfigurationException e) {
			e.printStackTrace();
			throw new JadxRuntimeException("CUNT");
		} catch (Exception e) {
			throw new JadxRuntimeException("Failed to load xml file: " + xml, e);
		}
		LOG.debug("SUCCESSFULLY [[sexed]] UR MUM (refer to previous comment that mentioned \"ur mum\" to know what \"ur mum\" is referring to)");
		return doc;
